...
export async function createCafeteriaComment({comment}) {
  return await cafeteriaCommentRepository.createCafeteriaComment(
    Entity.parse(comment, CafeteriaComment)
  );
}

// @ts-ignore
export async function updateCafeteriaComment({comment}) {
  return await cafeteriaCommentRepository.updateCafeteriaComment(
    Entity.parseFiltered(comment, CafeteriaComment)
  );
}
