...
class DiscountTransactionRepository extends SequelizeCRUDRepository<DiscountTransaction, DiscountTransactionModel> {
     ...
     * @param options
     */
    async getTransactions(options: {cafeteriaId: number, date: Date}) {
        return await this.readAll({
            where: {
                cafeteria_id: options.cafeteriaId,
                timestamp: DiscountTransactionRepository.getSameDayWhereClause(options.date)
            }
        });
    }

    private static getSameDayWhereClause(date: Date) {
        return {
